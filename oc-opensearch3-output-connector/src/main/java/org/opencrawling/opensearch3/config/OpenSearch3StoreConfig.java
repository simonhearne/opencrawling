 ...
 */
package org.opencrawling.opensearch3.config;
...
public class OpenSearch3StoreConfig {

    private static final Logger log = LoggerFactory.getLogger(OpenSearch3StoreConfig.class);
    ...
    public OpenSearchClient openSearchClient() {
        log.info("Initializing OpenSearchClient (3.x) connected to: {}", uris);
        try {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (!exists) {
                    ...
                    String mappingsJson = "{"
                            + "  \"properties\": {"
                            + "    \"" + OpenSearch3Constants.FIELD_ID + "\": { \"type\": \"keyword\" },"
                            + "    \"" + OpenSearch3Constants.FIELD_TEXT + "\": { \"type\": \"text\" },"
                            + "    \"" + OpenSearch3Constants.FIELD_URI + "\": { \"type\": \"keyword\" },"
                            + "    \"" + OpenSearch3Constants.FIELD_ACL + "\": { \"type\": \"keyword\" },"
                            + "    \"" + OpenSearch3Constants.FIELD_LAST_MODIFIED + "\": { \"type\": \"date\" },"
                            + "    \"" + OpenSearch3Constants.FIELD_SECURITY_INHERITANCE + "\": { \"type\": \"boolean\" },"
                            + "    \"" + OpenSearch3Constants.FIELD_SECURITY_ALLOWED_READ + "\": { \"type\": \"keyword\" },"
                            + "    \"" + OpenSearch3Constants.FIELD_SECURITY_DENIED_READ + "\": { \"type\": \"keyword\" },"
                            + "    \"" + OpenSearch3Constants.FIELD_EMBEDDINGS + "\": {"
                            + "      \"type\": \"knn_vector\","
