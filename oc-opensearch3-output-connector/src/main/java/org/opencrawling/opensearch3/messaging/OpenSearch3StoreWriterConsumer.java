...
public class OpenSearch3StoreWriterConsumer {

    private static final Logger log = LoggerFactory.getLogger(OpenSearch3StoreWriterConsumer.class);
    ...
    public OpenSearch3StoreWriterConsumer(OpenSearchClient client) {
        this.client = client;
    }

    @jakarta.annotation.PostConstruct
    public void init() {
        log.info("OpenSearch3StoreWriterConsumer (3.x) initialized successfully!");
    }
    ...
    public void consume(DocumentEmbeddedMessage message) {
        ...
        try {
            ...
            row.put(OpenSearch3Constants.FIELD_ID, message.chunkId());
            row.put(OpenSearch3Constants.FIELD_TEXT, message.text());
            
            Object uriVal = message.metadata().get("uri");
            row.put(OpenSearch3Constants.FIELD_URI, uriVal != null ? String.valueOf(uriVal) : "");

            Object aclVal = message.metadata().get("acl");
            row.put(OpenSearch3Constants.FIELD_ACL, aclVal != null ? String.valueOf(aclVal) : "");

            Object lastModifiedVal = message.metadata().get("lastModified");
            row.put(OpenSearch3Constants.FIELD_LAST_MODIFIED, lastModifiedVal != null ? String.valueOf(lastModifiedVal) : "");
            ...
            row.put(OpenSearch3Constants.FIELD_EMBEDDINGS, message.embedding());
